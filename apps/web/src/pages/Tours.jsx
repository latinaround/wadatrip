  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-900 via-slate-800 to-slate-700 text-white">
      <div className="mx-auto max-w-6xl px-4 py-12 space-y-6">
        ...
        {loading && <p className="text-slate-200">Cargando tours...</p>}
        {error && <p className="text-red-200">{error}</p>}

        {!loading && !error && items.length === 0 && (
          <p className="text-slate-200">Aun no hay tours publicados.</p>
        )}
        ...
        <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
          {items.map((item) => (
            <Link
              key={item.id}
              to={`/tours/${item.id}`}
              className="rounded-2xl border border-white/10 bg-white/10 p-5 shadow-lg transition-transform hover:-translate-y-1"
            >
              <div className="space-y-3">
                <div>
                  <p className="text-xs uppercase text-slate-300">{item.city || '-'}</p>
